...
For anything that isn't a bug, use the contact page.

Installing
You need an Apple Silicon Mac on macOS 14 or newer.

Once your request is approved, you receive an email with a link to your license key. The page asks you to confirm before it shows the key, so a mail scanner or a link preview can't reveal it. The link works for 24 hours; the key it shows is yours for good, so save it somewhere durable.
The same page offers the installer. Download the .dmg, open it, drag Catallaxy to /Applications, and eject the image.
Launch Catallaxy. The first launch asks for your license key, then walks you through connecting your session tool and summoning a first council.
...
Lost your link? Ask for a new one.

Uninstalling
In Catallaxy, choose Session → Prepare for Uninstall…. It removes Catallaxy's entries from your Claude Code settings. (The same menu item restores them if you change your mind before step 3.)
Quit the app and move Catallaxy.app from /Applications to the Trash.
Remove the data directory: rm -rf ~/.catallaxy/
...
The full Terms of Service that you accepted at download time are the binding version. This README provides a plain-language summary for your reference; where the two diverge, the Terms of Service govern.
...
Publish content you authored to the Freehold marketplace, subject to the Freehold marketplace terms below.
...
Privacy
Catallaxy is local-first by design: your transcripts, observer responses, and code never reach any infrastructure we operate. What the app sends to catallaxy.app and what the backend keeps is enumerated in the Terms, section 6, and answered in plain language on the FAQ. If you find a gap between those pages and what the app does, file a bug; we treat privacy claims as load-bearing.
...
We may update these terms during the closed beta. The effective date on the Terms of Service is authoritative. Material changes will be communicated to invited testers directly.
...
Source code
The Catallaxy source is closed during the beta; this repository holds no code. The application binary is signed with our Apple Developer ID and notarized by Apple (how to check: Installing).

Third-party software
Catallaxy is built on substantial open-source work. A complete list of the libraries we use, their versions, and their license texts is at THIRD-PARTY-LICENSES.md.

The biggest contributors include:

Tauri (Apache 2.0 / MIT) — the application framework.
Svelte (MIT) — the UI runtime.
Rust and the Rust ecosystem (various permissive licenses) — the desktop logic.
Elixir and the Elixir ecosystem (various permissive licenses) — the backend logic.
...
This product also showcases a lot of my own personal adoration of many cultural icons from my formative years (looking at you, Arthur!). My use of these references is entirely for educational and entertainment purposes, and with love and gratitude. If the owners of any characters that resemble the ones I have in this app have a problem with that, just send me a note through the contact page and I'll remove them.
